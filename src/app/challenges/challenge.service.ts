    ...
    return this.authService.user.pipe(take(1),
      switchMap(currentUser => {
        ...
        return this.http.get<{
          ...
          year: number;
          _days: Day[];
        }>(`https://udemy-ng-http-bfb93.firebaseio.com/challenge/${currentUser.id}.json?auth=${currentUser.token}`)
      }),
      tap(resData => {
        if (resData) {
  ...
  private saveToServer(challenge: Challenge) {
    this.authService.user.pipe(take(1),
      switchMap(currentUser => {
        ...
        return this.http.put(`https://udemy-ng-http-bfb93.firebaseio.com/challenge/${currentUser.id}.json?auth=${currentUser.token}`, challenge)
      }))
      .subscribe(res => {
        console.log(res);
      });
